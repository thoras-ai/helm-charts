...
Standard NetworkPolicy cannot target the API server by label, so with
networkPolicy.apiServerCIDRs empty this permits egress to any destination on
the configured ports. Setting apiServerCIDRs scopes the rule to those ipBlocks
instead. Policy is enforced after kube-proxy DNATs the service address to the
real endpoint, so the ports must match what the API server actually listens on
rather than the service port. The cilium flavor scopes by identity and does not
use this helper.
...
The first rule targets an in-cluster CoreDNS labeled k8s-app=kube-dns in
kube-system. networkPolicy.allowDnsToAnyDestination adds a second, ports-only
rule for clusters where that selector matches nothing: NodeLocal DNSCache
(GKE, EKS) answers on a link-local address owned by the node rather than a
pod, and some distributions label CoreDNS differently.
*/}}
...
fallback targets the host and remote-node entities, where a NodeLocal DNSCache
listener runs.
*/}}
...
{{/*
Ingress rule allowing a metrics scraper in any namespace to reach a
component's Prometheus port. Components enable prometheus by default and
Prometheus normally runs outside the release namespace, which the
same-namespace ingress rule does not cover. Emits nothing when
networkPolicy.allowMetricsScraping is false or the component has prometheus
disabled.

Not used by the API server, whose metrics share its API port.
...
Emits nothing for the bundled TimescaleDB, which the in-namespace rule covers.
*/}}
...
Resolution for every chart-managed credential, one entry per logical value:
  ...
  mode: existing  read from a Secret the customer manages
        values    pinned in values, stored in `thoras-helm-values`
...
the plaintext, present only for mode=values. `generate`/`migrateFrom` are the
config-controller seeding spec, present only for mode=seed. Values whose
feature is off are omitted entirely.

Sole source of truth for the ref helpers, `thoras-helm-values`, the
...
{{- $plan = append $plan (dict "name" "api-client-secret" "mode" "existing" "secret" $ac.existingSecret.secretName "key" $ac.existingSecret.secretKey) -}}
{{- else if $ac.secret -}}
...
{{- $plan = append $plan (dict "name" "dashboard-auth-password" "mode" "existing" "secret" $existing.secretName "key" $existing.passwordKey) -}}
{{- $plan = append $plan (dict "name" "dashboard-auth-cookie-secret" "mode" "existing" "secret" $existing.secretName "key" $existing.cookieSecretKey) -}}
{{- else -}}
{{- if $auth.htpasswd.password -}}
{{- $plan = append $plan (dict "name" "dashboard-auth-password" "mode" "values" "secret" "thoras-helm-values" "key" "dashboard-auth-password" "value" $auth.htpasswd.password) -}}
{{- else -}}
...
{{- $plan = append $plan (dict "name" "dashboard-auth-cookie-secret" "mode" "values" "secret" "thoras-helm-values" "key" "dashboard-auth-cookie-secret" "value" $auth.htpasswd.cookieSecret) -}}
{{- else -}}
...
{{- $plan = append $plan (dict "name" "dashboard-oidc-client-id" "mode" "existing" "secret" $existing.secretName "key" $existing.clientIDKey) -}}
{{- $plan = append $plan (dict "name" "dashboard-oidc-client-secret" "mode" "existing" "secret" $existing.secretName "key" $existing.clientSecretKey) -}}
{{- $plan = append $plan (dict "name" "dashboard-oidc-cookie-secret" "mode" "existing" "secret" $existing.secretName "key" $existing.cookieSecretKey) -}}
{{- end -}}
...
{{- $plan = append $plan (dict "name" "slack-webhook-url" "mode" "existing" "secret" .Values.slackWebhookUrlSecretRefName "key" .Values.slackWebhookUrlSecretRefKey) -}}
...
{{/*
Keys stored in `thoras-helm-values`: every logical value pinned in values.
Empty when the customer pinned nothing.
*/}}
{{- define "thoras.helmSecretValuesKeys" -}}
{{- $keys := dict -}}
...
{{/*
Body of `config-controller.yaml`. Mirrors thoras.secretPlan: existing and
values-pinned entries become `source: provided` file reads, seeded values
carry their generator and legacy migration sources.
*/}}
